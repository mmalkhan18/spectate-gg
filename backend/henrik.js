...
async function getAccountByRiotId(riotId, region) {
  ...
  const { data } = await axios.get(`${BASE}/v1/account/${encodeURIComponent(name)}/${encodeURIComponent(tag)}`, {
...
async function getRecentMatches(puuid, region) {
  const { data } = await axios.get(`${BASE}/v4/by-puuid/matches/${region}/pc/${puuid}?mode=competitive&size=1`, {
    headers: { Authorization: HENRIK_KEY }
  })
  return data.data || []
}
